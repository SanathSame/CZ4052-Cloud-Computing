...
class FramesGenerator(keras.utils.Sequence):
    ...
    def __init__(self, sPath:str, \
        ...
        seLabels =  self.dfVideos.sFrameDir.apply(lambda s: re.split(r"[/\\]", s)[-2])
...
class FeaturesGenerator(keras.utils.Sequence):
    ...
    def __init__(self, sPath:str, nBatchSize:int, tuXshape, \
        ...
        # extract (text) labels from path
        seLabels =  self.dfSamples.sPath.apply(lambda s: re.split(r"[/\\]", s)[-2])
...
class VideoClasses():
    ...
    def __init__(self, sClassFile:str):
        ...
        # sort the classes
        self.dfClass = self.dfClass.sort_values("gloss").reset_index(drop=True)
        
        self.liClasses = list(set(self.dfClass['gloss']))
        self.nClasses = len(self.liClasses)
