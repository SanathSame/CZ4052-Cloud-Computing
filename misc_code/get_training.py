...
from keras.layers import Input, Add, Activation, AveragePooling2D, MaxPooling2D, GlobalMaxPooling2D, \
    GlobalAveragePooling2D, TimeDistributed, LSTM, concatenate
...
def get_model():
    input_t = Input(shape=INPUT_SIZE)
    model_res = ResNet50V2(include_top=False, input_tensor=input_t, weights='imagenet', classes=28)
...
def get_simple_model():
    model = Sequential()
    model.add(Conv2D(128 , (3,3) , strides=1 , padding='same' , activation='relu' , input_shape=INPUT_SIZE))
...
def train_model():
    ...
    model.fit(norm_train_dataset, validation_data=norm_val_dataset, epochs=15, verbose=1, callbacks=[checkpoint])
    model.save("models/simple_model2_size28_epoch15.h5")
...
def test_model():
    ...
    model = tf.keras.models.load_model('models/simple_model2_size28.h5')
